...
import { Star, Navigation, Wifi, Coffee, Tv, CreditCard, School as Pool, Dumbbell } from 'lucide-react';

const HotelCard: React.FC<HotelCardProps> = ({ hotel, isSelected, onSelect }) => {
  // Determine the amenity icons based on the amenities array
  ...
  return (
    <div 
      className={`bg-white rounded-lg shadow-card hover:shadow-card-hover transition-all duration-300 p-4 cursor-pointer border-l-4 ${isSelected ? 'border-accent-500 bg-accent-50' : 'border-transparent'}`}
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className={`${priceDisplay.color} font-medium`}>
          {priceDisplay.text}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex flex-col text-sm text-neutral-600">
          ...
          <div className="text-neutral-500">
            {hotel.walking_time.toFixed(1)} min walk
          </div>
        </div>
